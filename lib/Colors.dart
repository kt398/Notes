import 'package:flutter/material.dart';
import 'dart:io';


//darks
const Color darkPrimary=Color(0xffBB86FC);
const Color darkPrimaryVariant=Color(0xff3700B3);
const Color darkSecondary=Color(0xff03DAC6);
const Color darkBackground=Color(0xff121212);
const Color darkSurface=Color(0xff121212);
const Color darkError=Color(0xffCF6679);
const Color darkonPrimary=Color(0xff000000);
const Color darkonSecondary=Color(0xff000000);
const Color darkonBackGround=Color(0xffffffff);
const Color darkonSurface=Color(0xffffff);
const Color darkonError=Color(0x000000);







//lights
const Color lightPrimary=Color(0xff6200EE);
const Color lightPrimaryVariant=Color(0xff018786);
const Color lightSecondary=Color(0xff03DAC6);
const Color lightSecondaryVariant=Color(0xff018786);
const Color lightBackground=Color(0xFFFFFFFF);
const Color lightSurface=Color(0xFFFFFFFF);
const Color lightError=Color(0xFFB00020);
const Color lightonPrimary=Color(0xFFFFFFFF);
const Color lightonSecondary=Color(0xff000000);
const Color lightonBackGround=Color(0xff000000);
const Color lightonSurface=Color(0xff000000);
const Color lightonError=Color(0xffffffff);
