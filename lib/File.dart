...
class Task extends Files {
  ...
  Task(String path, String title, DateTime dateModified, File file, String data,
      String textData, this.notificationID, this.date)
  ...
  Future<bool> renameFile(String newName, Folder parentFolder) async{
    ...
    entity=await entity.rename('${entity.parent.path}/$newName^&@${this.date.toIso8601String()}^&@${this.notificationID}');
    return true;
  }

  Future<void> changeDate(DateTime newDate)async{
    this.date=newDate;
    entity=await entity.rename('${entity.parent.path}/${this.title}^&@${this.date.toIso8601String()}^&@${this.notificationID}');
  }
    ...
    if (await fileExists(name)) {
      ...
      switch (t) {
        ...
        case Type.task:
          {
            File f = await File(
                    '${entity.path}/$name.json^&@${DateTime.now()}^&@${lastTask + 1}')
                .create(recursive: false);
            this.list.add(new Task(f.path, name, (await f.lastModified()), f,
                await f.readAsString(), '', lastTask + 1, DateTime.now()));
            print(f.path);
          }
          break;
      }
